...
for s in sqrs:
    for g in rows + cols:
        ...
        if m:
            ...
            b2 = m & ~ (m - 1)
            b3 = m & ~ b2
            alignments[b1 | b2] = alignments[
                b1 | b3] = alignments[b2 | b3] = (s, g)
...
def searchPlacements(node, census):
    ...
    for i in range(9):
        ...
        FullPlacementGraph[node][node | b] = 1 << (
            9 * i + sum(census))   # found edge
        census[i // 3] += 1
...
for row in range(9):
    for col in range(9):
        cellnames[
            row * 9 + col] = ''.join(['R', str(row + 1), 'C', str(col + 1)])
...
class Sudoku:
    ...
    def place(self, digit, cell, explanation=None):
        ...
        if digit != int(digit) or not 1 <= digit <= 9:
            raise ValueError(
                "place(%d,%d): digit out of range" % (digit, cell))
    ...
    for d in digits:
        for g in groups:
            ...
            if dglocs & (dglocs - 1) == 0:
                if dglocs == 0:
                    grid.log(
                        ["Unable to place", d, "anywhere in", g.name + "."])
...
def eliminate(grid):
    ...
    for cell in range(81):
        if not grid.contents[cell]:
            ...
            if len(allowed) == 0:
                grid.log(
                    ["Unable to place any digit in", cellnames[cell] + "."])
...
def explain_pair(grid, digs, locs):
    ...
    for g in g1:
        ...
        ing = ["In", (g1 and g1[0].name or "no group") + ",", "digit",
               str(d1) + ",",
               "and in", (g2 and g2[0].name or "no group") + ",", "digit",
               str(d2)]
                ...
                if d not in pairs.setdefault(dglocs, [d]):
                    ...
                    for e in digits:
                        if e not in pairs[dglocs]:
                            def explain():
                                return explain_pair(grid, pairs[dglocs],
                                                    dglocs)
...
def triad(grid):
    ...
    for mask, sqr, grp in triads:
        forces = [d for d in digits
                  ...
                  or (grid.locations[d] & grp.mask
                      == grid.locations[d] & mask)]
        if len(forces) == 3:
            ...
            for d in digits:
                def explain():
                    ...
                    elif (grid.locations[d] & sqr.mask
                          == grid.locations[d] & mask):
                        og = grp.name
                    else:
                        og = sqr.name
        ...
        if locs != mask:
            def explain():
                ...
                if masked & ~ (masked - 1):
                    ...
                    ex = "This cell is"
                return ex + (" not part of any valid placement of this digit"
                             " that covers each row, column and 3x3 square"
                             " exactly once.")
        ...
        if not (corners & grid.original_cells):
            ...
            for d in digits:
                ...
                if locs_for_d and not (locs_for_d & (locs_for_d - 1)):
                    if universal is not None or locs_for_d in candidates:
                ...
                if (cell, d) in grid.bilocation.reachable(cell, d):
                    ...
                    def explain():
                        expl = [incyclic, pathname(cycle) + ",",
                                bilocal_explanation + ".",
                                "If", d, "were not placed in", cellnames[
                                    cell] + ",",
                        ...
                        if len(cycle) == 4:
                            ...
                            expl += ["cycle's remaining", len(cycle) - 3,
                                     "digits",
                                     "in the remaining"]
                            if len(cycle) == 5:
                                expl.append("cell.")
...
def explain_conflict_path(grid, cell, d, why, reached, dd):
    ...
    if why[reached, dd]:
        ...
        return [inpath, pathname(path) + ",", bilocal_explanation + ".",
                ...
                "then", dd, "would have to be placed in", cellnames[
                    reached] + ",",
...
def createimplication(mask, pos, digit, T):
    """Add to 2SAT instance a constraint that the given digit in any
    mask position forces the same digit at pos.
...
def pseudonishio(g1, g2, grid, T):
    """Create 2SAT constraints for block-block interactions involving a
    single digit.

    If placing the digit in some cell of one block eliminates all but
    one of the potential locations for the same digit in another block,
    create an implication between the initial cell and the remaining
    location.
                ...
                if g.mask & (1 << cell):
                    ...
                    while candidates:
                        ...
                        if len(ch) == 3:
                            ...
                            for dd in [x, y]:
                                other = grid.locations[dd] & g.mask & ~ (
                                    1 << cell) & ~ (1 << candidate)
                                while other:
                ...
                for d in range(3):
                    ...
                    if grid.contents[cell]:
                        print(
                            ...
                            ' text-align:center; color:black">%d</td>'
                            % grid.contents[cell])
                    else:
                        print(
                            '<td width=30 height=30 align=center'
                            ' valign=middle><input style="font-family:times,'
                            'serif; font-size:16pt; text-align:center;'
                            ' color:#555; margin:0pt; border-width:0" size=1'
                            ' maxlength=1></td>')
                print("</tr>")
            print("</table></td>")
        print("</tr>")
...
def svg_format(grid):
    ...
    for i in [1, 2, 4, 5, 7, 8]:
        ...
        svg.segment(pos + 2j, pos + 272j)
    svg.ungroup()
    svg.group(style={"font-family": "Times", "font-size": "24",
    ...
    for row in range(9):
        for col in range(9):
            ...
            if grid.contents[cell]:
                svg.text(
                    str(grid.contents[cell]), 30 * col + 17 + 30j * row + 25j)
    svg.ungroup()
    svg.close()
    return False
...
def all_solutions(grid, fastrules=True):
    ...
    while True:
        ...
        grid.unplace(d, 1 << c, "The backtracking search has already tried"
                     " this placement, and now must try the opposite"
                     " decision.")
...
if __name__ == '__main__':
    ...
    if args:
        print("Unrecognized command line syntax, use --help for input"
              " documentation")
        sys.exit(0)
